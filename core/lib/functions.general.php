 ...
 *	Copyright (C) 2021 geteso.org.  <http://geteso.org>
 *
...
/**
 * General functions. Contains utility functions that are used throughout
 * the application, such as sanitation functions, color functions, JSON
 * encoding, URL construction and array manipulation.
 *
 * @package eso
 */
 ...
 * @param string $newFunction The name of a function that should be used
 * 		instead.
 * @return void
 *
 * @package eso
...
/**
 * Translate a string to its normal form or its plurular form, depending
 * on an amount.
 *
...
/**
 * Get a request input value, falling back to a default value if it is not
 * set. POST will be searched first, then GET, and then the fallback will
 * be used.
 *
...
/**
 * Write contents to a file, attempting to create the directory that the
 * file is in if it does not exist.
 *
...
/**
 * Converts the php.ini notiation for numbers (like '2M') to an integer of
 * bytes.
 *
...
function sendEmail($to, $subject, $body)
{
	try {
		...
		$mail->IsHTML(true);
		$mail->AddAddress($to);
		$mail->SetFrom(C("eso.emailFrom"), sanitizeForHTTP(C("eso.forumTitle")));
...
/**
 * Parse an array of request parts (eg. $_GET["p"] exploded by "/"), work
 * out what controller to set up, instantiate it, and work out the method
 * + arguments to dispatch to it.
 *
 * @param array $parts An array of parts of the request.
 * @param array $controllers An array of available controllers, with the
 * 		keys as the controller names and the values as the factory names.
...
function parseRequest($parts, $controllers)
{
	...
	// Determine the controller method and response type to use. Default
	// to index.
	$arguments = array_slice($parts, 2);
	if (!empty($parts[1])) {
		$method = strtolower($parts[1]);

		// If there's a period in the method string, use the first half as
		// the method and the second half as the response type.
...
function isMobileBrowser()
{
	...
	if (is_null($isMobileBrowser)) {
		...
		// This code is from http://detectmobilebrowser.com/ by Chad Smith.
...
/**
 * Create a slug for use in URLs from a given string.
 * Any non-alphanumeric characters will be converted to "-".
 *
...
{
	// If there are any characters other than basic alphanumeric, space,
	// punctuation, then we need to attempt transliteration.
	if (preg_match("/[^\x20-\x7f]/", $string)) {
	
		// Thanks to krakos for this code!
		if (function_exists('transliterator_transliterate')) {

			// Unicode decomposition rules states that these cannot be
			// decomposed, hence we have to deal with them manually.
			//
			// Note: even though “scharfes s” is commonly transliterated
			// as “sz”, in this context “ss” is preferred, as it's the
			// most popular method among German speakers.
			$src = array('œ', 'æ', 'đ', 'ø', 'ł', 'ß', 'Œ', 'Æ', 'Đ', 'Ø', 'Ł');
			$dst = array('oe','ae','d', 'o', 'l', 'ss', 'OE', 'AE', 'D', 'O', 'L');
			$string = str_replace($src, $dst, $string);

			// Using transliterator to get rid of accents and convert
			// non-Latin to Latin.
			...
			// Convert special Latin letters and other characters to HTML
			// entities.
			$string = htmlentities($string, ENT_NOQUOTES, "UTF-8");

			// With those HTML entities, either convert them back to a
			// normal letter, or remove them.
	...
	// Now replace non-alphanumeric characters with a hyphen, and remove
	// multiple hyphens.
...
/**
 * Generate a salt of $numOfChars characters long containing random
...
/**
 * For bad server configs. Pretty much just performing stripslashes on an
 * array here.
 *
...
/**
 * For bad server configs as well. Unset all input variables added to the
 * global namespace if register_globals is on.
 *
 * @return void
 *
 * @package eso
 ...
 * @param bool $normalize Whether or not the values of the RGB triplet
 * 		should be 0-255 or 0-1.
 * @return array The RGB triplet.
 *
 * @package eso
 */
 ...
 *
 * @param array $rgb The RGB triplet.
 * @param bool $normalize Whether or not the values of the RGB triplet are
 * 		0-255 or 0-1.
 ...
 * to a page, according to the format specified by C("eso.urls").
 *
 * @param string $url The request path (eg. conversations/all).
 * 		May include a query string/hash.
...
function URL($url = "", $absolute = false)
{
	...
	// If we don't have nice urls, use ?p=controller/method/argument
	// instead.
	if (!C("eso.urls.friendly") and $url) {
		$link = "?p=".$url;
		if ($query) $query[0] = "&";
	}
	else $link = $url;
	...
	// If we're not using mod_rewrite, we need to prepend "index.php/" to
	// the link.
	if (C("eso.urls.friendly") and !C("eso.urls.rewrite")) $link = "index.php/$link";
	return $absolute ? rtrim(C("eso.baseURL"), "/")."/".$link : getWebPath($link);
}


/**
 * Remove the absolute path to the root directory from a file path.
 *
 * @param string $path The file path.
 * @return string The path relative to the forum root directory.
 *
 * @package eso
 */
...
function getWebPath($path)
{
	if (strpos($path, "://") === false) {
		...
		// Remove the absolute path to the root directory.
...
/**
 * Get the relative path or URL to a resource (a web-accessible file
 * stored in plugins, skins, languages, or js) depending on the state of
 * the resourceURL config setting.
 *
...
function getResource($path, $absolute = false)
{
	if (strpos($path, "://") === false) {
		...
		// Prepend the web path.
		$path = ltrim($path, "/");
		if ($c = C("eso.resourceURL")) $path = $c.$path;
		else $path = $absolute ? rtrim(C("eso.baseURL"), "/")."/".$path : ET::$webPath."/".$path;
...
/**
 * Get a human-friendly string (eg. 1 hour ago) for how much time has
 * passed since a given time.
 *
 * @param int $then UNIX timestamp of the time to work out how much time
 * 		has passed since.
 * @param bool $precise Whether or not to return "x minutes/seconds", or
 * 		just "a few minutes".
...
{
	// If there is no $then, we can only assume that whatever it is never
	// happened...
	...
	// If we're going for a precise value, go on to test at the
	// minute/second level.
	if ($precise) {
...
function smartTime($then, $precise = false)
{
	...
	// If the time was within the last 48 hours, show a relative time
	// (eg. 2 hours ago.)
	if ($ago >= 0 and $ago < 48 * 60 * 60) return relativeTime($then, $precise);

	// If the time is within the last half a year or the next half a year,
	// show just a month and a day.
...
/**
 * Extract the contents of a ZIP file, and return a list of files it
 * contains and their contents.
 *
...
/**
 * Add an element to an keyed array after/before a certain key, or at a
 * certain index.
 *
 * @param array $array The array to add to.
 * @param string $key The key to add to the array.
 * @param mixed $value The value to add to the array.
 * @param mixed $position The position to add the element at. If this is
 * 		an integer, the element will be added at that index. If this is
 * 		an array with the first key as "before" or "after", the element
 * 		will be added before or after the specified key.
 * @return void
 *
 * @package esoTalk
 */
function addToArrayString(&$array, $key, $value, $position = false)
{
	...
	// If the position is "before" or "after" a certain key, find that
	// key in the array and record the index.
	// 
	// If the key doesn't exist, then we'll add the element to the end of
	// the array.
	...
	// Add the key/value to their respective arrays at the appropriate
	// index.
