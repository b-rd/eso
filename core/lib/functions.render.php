...
/**
 * Render functions. Contains functions that generally return common
 * elements to be rendered to the page, such as avatars, member profile
 * links, member names, group names, and stars.
 * 
 * All of the functions in this file are wrapped with function_exists
 * checks, so plugins and skins can override them in their plugin.php or
 * skin.php files if needed.
 *
 * @package eso
 */
...
/**
 * Find a list of words in a block of text and put spans with
 * class='highlight' around them.
 *
...
function highlight($text, $words)
{
	...
	foreach ($words as $word) {
		...
		// Make sure we only highlight whole words that would've been
		// matched by a fulltext search.
 ...
 * @return string The URL to the conversation (to be used in the URL
 * 		function.)
 *
 * @package eso
 */
...
/**
 * Construct a URL to a member, given their ID and username, and the
 * profile pane to go to.
 *
 * @param int $memberId The ID of the member.
 * @param string $username The member's username.
 * @param string $pane The profile pane to go to.
 * @return string The URL to the member's profile (to be used in the URL
 * function.)
 *
 * @package eso
 */
 ...
 * @return string The URL to the search page (to be used in the URL
 * 		function.)
 *
 * @package eso
 */
...
{
	return "conversations/".($search ? "?search=".urlencode($search) : "");
}
...
/**
 * Return a member's name wrapped in an anchor tag linking to their
 * profile page.
 *
 * @param int $memberId The ID of the member.
...
/**
 * Return a member's name to be displayed in an HTML context. If the name
 * is blank, it is assumed the member has been deleted, and "[deleted]"
 * is returned.
 *
 * @param string $username The member's username.
 * @param bool $sanitize Whether or not to sanitize the name for HTML
 * 		output.
 * @return string
 *
 * @package eso
 ...
 *
 * @param array $member An array of the member's details. (memberId and
 * 		avatarFormat are required in this implementation.)
 ...
 * @param bool $showMember Whether or not to show "Member" if the member's
 * 		account is Member and they aren't in any groups, or to just show
 * 		nothing.
 * @return string
 *
 * @package eso
...
{
	// If the member isn't a Member, groups don't matter - just display
	// their account type.
	if ($account and $account != ACCOUNT_MEMBER) return "<span class='group-$account'>".groupName($account)."</span>";
	else {

		// Otherwise, show a comma-separated list of the groups that
		// they're in.
		...
		// If they're not in any groups, either show them as a "Member" or
		// just show nothing at all.
 ...
 * @param bool $plural Whether or not a plural version of the name should
 * 		be used (if such a translation exists.)
 * @return string
 *
 * @package eso
 */
...
/**
 * Return a group's name wrapped in an anchor linking to a list of members
 * in that group.
 *
 * @param string $group The name of the group.
 * @return string
...
/**
 * Return a star for a certain conversation that can be clicked to toggle
 * the starred that of that conversation.
 *
 * @param int $conversationId The ID of the conversation that this star is
 * 		for.
 * @param bool $starred Whether or not the conversation is currently
 * 		starred.
 * @return string
 *
 * @package eso
...
/**
 * Return a star BUTTON for a certain conversation that can be clicked to
 * 		toggle the starred that of that conversation.
 *
 * @param int $conversationId The ID of the conversation that this star is
 * 		for.
 * @param bool $starred Whether or not the conversation is currently
...
{
	// Make sure the ETConversationModel class has been loaded so we can
	// access its static properties.
